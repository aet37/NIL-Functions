...
for mm=2:length(t),
  ...
  if (ftype==2),
    ...
    elseif (ctype==3),
      ...
      Fout(mm-1) = F0*( Vk2*(exp(Vk1*(VV(mm-1)/V0-1))-1)+1 ) + Vk3*(VV(mm)-VV(mm-1))/dt;
    elseif (ctype==4),
  ...
  EEx(mm) = 1 - (1-EEx(1))^(F(1)/F(mm-1));

  qq(mm) = qq(mm-1) + dt*( Ca*EE(mm-1)*F(mm-1) - qq(mm-1)*Fout(mm-1)/VV(mm-1) );
