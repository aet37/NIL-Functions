function [xx,y1,u1]=linAmp2(x,t,foh_args,hrf,data,tbias)
% Usage ... [xnew,yest,uu1]=linAmp2(x,t,foh_args,hrf,data,tbias_or_ww)
%
...
tmpioff=length(ttfoh_iv)+1;
if isempty(x),
  x=ones(1,tmpioff*size(hrf,2));
end;

x0=ones(size(x));
for mm=1:size(hrf,2),
  if mm==1,
    xlb=[ 0    -1*ones(1,tmpioff-1)];
    xub=[+1e4  +1*ones(1,tmpioff-1)];
  else,
    xlb=[xlb   0    -1*ones(1,tmpioff-1)];
    xub=[xub  +1e4  +1*ones(1,tmpioff-1)];
  end;
end;

if nargin>4,
  xx=lsqnonlin(@linAmp2_wrap,x0,xlb,xub,xopt,t,foh_args,hrf,data,ibias);
else,
  xx=x;
end;
...
function [y,u,uu]=linAmp2_wrap(x,t,foh_args,hrf,data,ibias)
  ttfoh=foh_args{1};
  ttfoh_iv=foh_args{2};
  ttfoh_if=foh_args{3};
  ...
  tmpioff=length(ttfoh_iv)+1;
  for nn=1:size(hrf,2),
    u(:,nn)=myfoh(ttfoh,ttfoh_iv,x([1:tmpioff-1]+(nn-1)*tmpioff+1),ttfoh_if,0,t); 
    uu(:,nn)=x((nn-1)*tmpioff)+1)*u(:,nn)/sum(u(:,nn));
    yest=yest+myconv(uu(:,nn),hrf(:,nn));
  end;
  y=yest;
  ...
  if nargin>4,
    ...
    if do_penalty,
      beta=2e-3;
      y=y.^2+beta*mean(abs(diff(x(2:end))));
    end;
    if length(ibias)==2, 
