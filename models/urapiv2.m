function [varargout] = urapiv(fname,itt,spc,s2nm,s2nl,sclt,outl,crop_vector,mask)
...
% Usage:
%
% 				UraPIV(FNAME,ITT,SPC,S2NM,S2NL,SCLT,OUTL,CROPV,MASK)
%
% Inputs:
%
...
if nargin>8
  [a,b] = read_pair_of_images(fname,[1 2],[0 0 0 0],itt,spc,ext,mask);
else,
  [a,b] = read_pair_of_images(fname,[1 2],[0 0 0 0],itt,spc,ext);
end
[sx,sy]= size(a);
...
%for fileind = 1:amount-1	% main loop, for whole file list
for fileind = 1:2:2*amount-1	% main loop, for whole file list
    ...
    if (nargin>8),
      [a,b] = read_pair_of_images(fname,[fileind fileind+1],crop_vector,itt,spc,ext,mask);
    else,
      [a,b] = read_pair_of_images(fname,[fileind fileind+1],crop_vector,itt,spc,ext);
    end
    [sx,sy]= size(a);
    ...
    fid = fopen([fname(1:end-4),'_',num2str(fileind),'_noflt.txt'],'w');
    ...
    fid = fopen([fname(1:end-4),'_',num2str(fileind),'_flt.txt'],'w');
    fprintf(fid,'%3d %3d %7.4f %7.4f %7.4f\n',filt_res');
    fclose(fid);
    end
    ...
    fid = fopen([fname(1:end-4),'_',num2str(fileind),'.txt'],'w');
    fprintf(fid,'%3d %3d %7.4f %7.4f %7.4f\n',res');
    fclose(fid);
    end
    ...
    cntcnt=cntcnt+1;
    resres(cntcnt,1,:,:)=no_filt_res;
    resres(cntcnt,2,:,:)=filt_res;
    resres(cntcnt,3,:,:)=res;
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [A,B] = read_pair_of_images(image1,image2,crop_vector,itt,spc,ext,mask)
	...
	A = imread(image1,image2(1));
	B = imread(image1,image2(2));
	%A = double(A(:,:,1))/255;
	%B = double(B(:,:,1))/255;
	A = double(A); Amin=min(min(A)); Amax=max(max(A));
	B = double(B); Bmin=min(min(B)); Bmax=max(max(B));
	A = (A - Amin)/(Amax - Amin);
	B = (B - Bmin)/(Bmax - Bmin);
	if (nargin>6),
		A = A.*mask;
    ...
    function [filenames,amount,ext] = ReadImDir(directory,ext)
        ...
        knownExtensions = {'bmp','jpg','tif','tiff','jpeg','seq'};
