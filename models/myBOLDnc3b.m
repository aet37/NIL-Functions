...
% Usage ... [S,Fin,Fout,Vv,CMRO2,E,q,t]=myBOLD(x,tparms,sparms,parms2fit_i,data,tdata)
%
% x = parameters to fit
% tparms = [dt T]
...
for mm=2:length(t),
  ...
  elseif (ctype==4),
    VV(mm) = VV(mm-1) + dt*(1/(1+F0*Vk3))*( Fin(mm-1) - F0*(Vk4*((VV(mm-1)/V0)^(1/grubb) -1)+1) );
    Fout(mm) = Fin(mm) - (1/dt)*( VV(mm)-VV(mm-1) );
  else,
  ...
  dVm=0;
  CCm(mm) = CCm(mm-1) + (dt/Vm)*( -CCm(mm-1)*dVm + PSm*(CCt(mm-1) - CCm(mm-1)) - CMRO2t(mm-1) );  

  if (RK_flag),
    Cmk1=(dt/Vt)*( -CCm(mm-1)*dVm + PSm*( CCt(mm-1) - CCm(mm-1)) - CMRO2a );
    Cmk2=(dt/Vt)*( -(CCm(mm-1)+Cmk1/2)*dVm + PSm*( CCt(mm-1)- (CCm(mm-1)+Cmk1/2) ) - CMRO2b );
    Cmk3=(dt/Vt)*( -(CCm(mm-1)+Cmk2/2)*dVm + PSm*( CCt(mm-1)- (CCm(mm-1)+Cmk2/2) ) - CMRO2b );
    Cmk4=(dt/Vt)*( -(CCm(mm-1)+Cmk3)*dVm + PSm*( CCt(mm-1)- (CCm(mm-1)+Cmk3) ) - CMRO2c );
