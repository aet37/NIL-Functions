...
{
  /* input and output argument variable declarations */
  double t0,wex,klfp,Nlfp,atau,vtau,F0,Vv0,Pa,Pt,CMRO20;
  double P50,alphaO2,cHb,PO,Vc,Vt,Vct,kb;
  double *Stim,*fneu,*U,*Fin,*Fout,*Vv;
  double *Cc,*Cp,*Ct,*E,*CMRO2,*q;
  double *t,*Y,*Y2;
  ...
  /*  Get the sizes of all matrices input, to determine if proper input is given   */  
  /*  nechosm = mxGetM(prhs[0]);     
  nechosn = mxGetN(prhs[0]);
  TEm = mxGetM(prhs[1]);
  TEn = mxGetN(prhs[1]);
  T1m = mxGetM(prhs[2]);
  T1n = mxGetN(prhs[2]);
  ...
  typem = mxGetM(prhs[4]);
  typen = mxGetN(prhs[4]);
  degree_precisionm = mxGetM(prhs[5]);
  degree_precisionn = mxGetN(prhs[5]);
  funcm = mxGetM(prhs[6]);
  funcn = mxGetN(prhs[6]);
  ampn = mxGetM(prhs[7]);
  ampm = mxGetN(prhs[7]);
  nrampm = mxGetM(prhs[8]);
  nrampn = mxGetN(prhs[8]);*/
  ...
  nechos = mxGetScalar(prhs[0]); 
  TE = mxGetScalar(prhs[1]);
  T1 = mxGetScalar(prhs[2]);
  T2 = mxGetScalar(prhs[3]);
  type = mxGetScalar(prhs[4]);
  degree_precision = mxGetScalar(prhs[5]);
  func = mxGetScalar(prhs[6]);
  amp = mxGetScalar(prhs[7]);
  nramp = mxGetScalar(prhs[8]);
  
  
  /* Must create output matrix for values of yr and yi  */
  plhs[0] = mxCreateDoubleMatrix(nechos,1,mxREAL);
  y = mxGetPr(plhs[0]);
  
  etrain(y,nechos, TE, T1, T2, type, degree_precision, func, amp, nramp);
  myBOLDnc2b_mex(S,Fin,Fout,Vv,Cc,Cp,Ct,CMRO2,E,q,t,U4,U2,x,tparms,parms,parms2fit,data,tdata);
