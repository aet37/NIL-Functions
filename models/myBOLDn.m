function [S,F2,F4,V3,CMRO2,EE,q,t]=myBOLD(x,tparms,sparms,parms2fit_i,data,tdata)
...
ytau=3/60;
yamp=0.25;
y2st=sparms(5); y2dur=sparms(6); y2ramp=sparms(7:8);	% 3.5/60 11.5/60 0.5/60
y2tau=sparms(9);		% 2/60
y2amp=sparms(10);		% 0.62

cstart=sparms(5); cdur=sparms(6); cramp=sparms(7:8);
...
CMRO2=CMRO20*(1+camp*mytrapezoid(t,cstart,cdur,cramp,ytype));
...
for mm=2:length(t),
  ...
  elseif (c3type==4),
    y3(mm)=y3(mm-1)+dt*( (1/C3tau)*( mytrapezoid(t(mm-1),yst,ydur,yramp,ytype) - y3(mm-1) ));
  ...
  y2(mm) = y2(mm-1) + dt*( (1/y2tau)*( mytrapezoid(t(mm-1),y2st,y2dur,y2ramp,ytype) - y2(mm-1) ));
  ...
  if (RK_flag),
    y2h = y2(mm-1) + (dt/2)*( (1/y2tau)*( mytrapezoid(t(mm-1),y2st,y2dur,y2ramp,ytype) - y2(mm-1) ));
    Finh = F0*(1+y2amp*y2h);
    Finhh= F0*(1+y2amp*y2(mm));
  end;
  ...
  if (RK_flag),
    CMRO2a=CMRO20*(1+camp*mytrapezoid(t(mm-1),cstart,cdur,cramp,ytype));
    CMRO2b=CMRO20*(1+camp*mytrapezoid(t(mm-1)+dt/2,cstart,cdur,cramp,ytype));
    CMRO2c=CMRO20*(1+camp*mytrapezoid(t(mm-1)+dt,cstart,cdur,cramp,ytype));
  ...
  SB(mm-1) = 1 + sk1*( 1 - sk2*qB(mm-1)/qB(1) );
  SBs(mm-1) = 1 + sk1*( 1 - sk2*qBs(mm-1)/qBs(1) );
  SBb(mm-1) = 1 + sk1*( 1 - sk2*qBb(mm-1)/qBb(1) );
  %SBn(mm-1) = 1 + sk1*( 1 - sk2*qBn(mm-1)/qBn(1) );
