function [S,F2,F4,V3,CMRO2,EE,q,t]=myBOLD2(x,tparms,sparms,parms2fit_i,iunfix,ifix,tdata,data)
...
%Fin=F0*(1+yamp*myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t));
...
for mm=2:length(t),
  ...
  if (RK_flag),
    yh = y(mm-1) + (dt/2)*(1/ytau)*( myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)) - y(mm-1) );
  ...
  y2(mm) = y2(mm-1) + dt*( (1/y2tau)*( myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)) - y2(mm-1) ));
  
  %Fin(mm-1) = F0*( 1 + y2amp*y2(mm-1) );
  %Fin(mm) = F0*( 1 + y2amp*y2(mm) );
  ff=F0*(1+yamp*myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t));
  Fin(mm-1)=ff(mm-1);
  Fin(mm)=ff(mm);
  if (ctype==2),
