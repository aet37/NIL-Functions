function [xx,y1,u1]=linAmp2(x,t,foh_args,hrf,data,cbeta,tbias)
% Usage ... [xnew,yest,uu1]=linAmp2(x,t,foh_args,hrf,data,cbeta,tbias_or_ww)
%
...
if isempty(x),
  x=ones(1,length(ttfoh_iv)+1);
end;

x0=ones(size(x));
xlb=[ 0    -1*ones(1,length(x)-1)];
xub=[+1e4  +1*ones(1,length(x)-1)];
...
function [y,u,uu]=linAmp2_wrap(x,t,foh_args,hrf,data,cbeta,ibias)
  ...
  yest=zeros(size(hrf));
  u=myfoh(ttfoh,ttfoh_iv,x(2:end),ttfoh_if,0,t); 
  uu=u/sum(u);
  yest=yest+myconv(uu,x(1)*hrf);
  y=yest;
