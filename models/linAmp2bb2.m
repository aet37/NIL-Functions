function [xx,y1,u1]=linAmp2bb2(x,t,foh_args,hrf,data,cbeta,tbias)
% Usage ... [xnew,yest,uu1]=linAmp2bbb(x,t,foh_args,hrf,data,cbeta,tbias_or_ww)
%
...
if isempty(x),
  x=[100 ones(1,n_hrf) 2*rand(1,n_hrf*xlen1)];
end;

x0=ones(size(x));
xlb=[ 0   -1e2*ones(1,n_hrf) -1*ones(1,n_hrf*xlen1)];
xub=[+1e4 +1e2*ones(1,n_hrf) +1*ones(1,n_hrf*xlen1)];
...
function [y,uu]=linAmp2_wrap(x,t,foh_args,hrf,data,cbeta,ibias)
  ttfoh=foh_args{1};
  ttfoh_iv=foh_args{2};
  ttfoh_if=foh_args{3};
  xlen1=length(ttfoh_iv);
  ...
  for mm=1:n_hrf,
    uu(:,mm)=myfoh(ttfoh,ttfoh_iv,x(1+n_hrf+(mm-1)*xlen1+[1:xlen1]),ttfoh_if,0,t); 
    uu(:,mm)=uu(:,mm)/sum(uu(:,mm));
    yest=yest+x(mm)*myconv(uu(:,mm),hrf(:,mm));
  end;
  yest=yest*x(1);
  y=yest;
  ...
  if nargin>4,
    ...
    if do_penalty,
      beta=cbeta;    % 2e-3
      cpen=0;
      for mm=1:n_hrf, cpen=cpen+abs(diff(x(1+n_hrf+(mm-1)*xlen1+[1:xlen1]))); end;
      y=y.^2+beta*mean(cpen);
    end;
    if length(ibias)==2, 
