...
parms2fit=[3 6 7 8 9 11 12 13 14];		% used first to figure average
parms2fit=[3 6 7 8 9 13];			% officially used
%parms2fit=[3 6 8 9 13];
%parms2fit2=parms2fit;
%parms2fit2=[3 6 13];			% originally NIMG submission
parms2fit2=[3 13];			% remove t0 from fit list
doadjust=1;
...
xx1=lsqnonlin(@complG3f,xg1(parms2fit),xl1(parms2fit),xu1(parms2fit),opt2,tparms1,xg1,parms2fit,[avgC1;avgE1],tt1);
...
xx2=lsqnonlin(@complG3f,xg2(parms2fit),xl2(parms2fit),xu2(parms2fit),opt2,tparms2,xg2,parms2fit,[avgC2;avgE2],tt2);
...
xx2b=lsqnonlin(@complG3f,xg2b(parms2fit2),xl2(parms2fit2),xu2(parms2fit2),opt2,tparms2,xg2b,parms2fit2,[avgC2b;avgE2b],tt2);
...
xx3=lsqnonlin(@complG3f,xg3(parms2fit),xl3(parms2fit),xu3(parms2fit),opt2,tparms3,xg3,parms2fit,[avgC3;avgE3],tt3);
...
xx3b=lsqnonlin(@complG3f,xg3b(parms2fit2),xl3(parms2fit2),xu3(parms2fit2),opt2,tparms3,xg3b,parms2fit2,[avgC3b;avgE3b],tt3);
...
err1=sum((avgE1i-yy1(2,:)).^2)/length(avgE1i);
err2=sum((avgE2i-yy2(2,:)).^2)/length(avgE2i);
err2b=sum((avgE2bi-yy2b(2,:)).^2)/length(avgE2bi);
err3=sum((avgE3i-yy3(2,:)).^2)/length(avgE3i);
err3b=sum((avgE3bi-yy3b(2,:)).^2)/length(avgE3bi);

err1f=sum((avgC1i-yy1(1,:)).^2)/length(avgC1i);
err2f=sum((avgC2i-yy2(1,:)).^2)/length(avgC2i);
err2bf=sum((avgC2bi-yy2b(1,:)).^2)/length(avgC2bi);
err3f=sum((avgC3i-yy3(1,:)).^2)/length(avgC3i);
err3bf=sum((avgC3bi-yy3b(1,:)).^2)/length(avgC3bi);
...
legend('Hyperox Data','Hyperox Fit')
subplot(212)
...
figure(2)
subplot(211)
plot(tt2*60,avgC2,'b--',tt2*60,avgC2b,'m--',t2*60,yy2(1,:),'b-',t2*60,yy2b(1,:),'m-')
ylabel('FAIR'),
axis('tight'), grid('on'), fatlines, dofontsize(15);
legend('Hyperox Data','Hypercap Data','Hyperox Fit','Hypercap Fit')
subplot(212)
plot(tt2*60,avgE2,'b--',tt2*60,avgE2b,'m--',t2*60,yy2(2,:),'b-',t2*60,yy2b(2,:),'m-')
ylabel('BOLD'), xlabel('Time (s)'),
axis('tight'), grid('on'), fatlines, dofontsize(15);
legend('Hyperox Data','Hypercap Data','Hyperox Fit','Hypercap Fit')
...
figure(3)
subplot(211)
plot(tt3*60,avgC3,'b--',tt3*60,avgC3b,'m--',t3*60,yy3(1,:),'b-',t3*60,yy3b(1,:),'m-')
ylabel('FAIR'), xlabel('Time (s)'),
axis('tight'), grid('on'), fatlines, dofontsize(15);
legend('Hyperox Data','Hypercap Data','Hyperox Fit','Hypercap Fit')
subplot(212)
plot(tt3*60,avgE3,'b--',tt3*60,avgE3b,'m--',t3*60,yy3(2,:),'b-',t3*60,yy3b(2,:),'m-')
ylabel('BOLD'), xlabel('Time (s)'),
axis('tight'), grid('on'), fatlines, dofontsize(15);
legend('Hyperox Data','Hypercap Data','Hyperox Fit','Hypercap Fit')
