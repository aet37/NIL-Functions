...
slb=[30  1  1/60  -0.5 20 0.05 97.9  2000 80   0 0.00001];
sub=[120 10 40/60 +2.0 31 5.00 98.1  9000 300 50 0.01000];
sparms(11)=0.000;
...
if (dosparms),
  ...
  MC_SLB=slb;
  MC_SUB=sub;
  for mm=1:length(sparms),
    if (isempty(find(mm==parms2fit))),
      MC_SLB(mm)=sparms(mm);
      MC_SUB(mm)=sparms(mm);
    end;
  end;
  MC_SPARMS=rand(MC_N,length(sparms)).*(ones(MC_N,1)*(MC_SUB-MC_SLB)) + ones(MC_N,1)*MC_SLB;
  save myPO2res_mc -append MC*
else,
  load myPO2res_mc MC_N MC_SLB MC_SUB MC_SPARMS
end;

domc=0;
if (domc),
  for ii=2:MC_N,
    [MC_xx(ii,:)]=lsqnonlin(@myPO2est,MC_SPARMS(ii,parms2fit),MC_SLB(parms2fit),MC_SUB(parms2fit),xopt,tparms,MC_SPARMS(ii,:),parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBF6t-CBF6t(1))/60,[CBF6i(:) PO26f(:)]);
    [MC_CCt(ii,:),MC_CCc(ii,:),MC_CMRO2t(ii,:),t]=myPO2est(MC_xx(ii,:),tparms,MC_SPARMS(ii,:),parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBF6t-CBF6t(1))/60,[CBF6i(:)]);
    save myPO2res_mc
  end;
else,
  load myPO2res_mc MC_xx MC_CCt MC_CCc MC_CMRO2t
end;
...
if (doplot==1),
  ...
  axis('tight'), grid('on'), fatlines; dofontsize(15);
elseif (doplot==2),
  mean(std(MC_xx(:,end-length(iunfix)+1:end))),
  subplot(311)
  plot(CBF6t,CBF6i,'b')
  ylabel('CBF/CBF_0'),
  axis('tight'), grid('on'), fatlines; dofontsize(15);
  subplot(312)
  plot(PO26t,PO26f,'b',t*60+CBF6t(1),mean(MC_CCt)/mean(MC_CCt(:,1)),'g')
  ylabel('P_{tO2}/P_{tO20}'),
  axis('tight'), grid('on'), fatlines; dofontsize(15);
  legend('Data','Model')
  subplot(313)
  MC_CMRO2ti=interp1(t,MC_CMRO2t.',ttt).';
  errorbar(ttt*60+CBF6t(1),mean(MC_CMRO2ti)/mean(MC_CMRO2t(:,1)),2*std(MC_CMRO2ti),'g')
  ylabel('CMR_{O2}/CMR_{O20}'),
  axis('tight'), grid('on'), fatlines; dofontsize(15);
end;
