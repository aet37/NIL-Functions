...
sparms=[F0 Vv0 Vvk1 dCMRO2 P50 cHb Vc Vct PS  Pa Pt bbeta fp2cmro2f Vm PSm Vmamp Caamp];
slb=[30  1  1/60  +0.03 20 1.5 0.1 47.9  2000 80   0 0.00000 0.0 1.0 100 -1.0 -0.5];
sub=[25  100 40/60 +0.60 40 3.5  50.00 498.1  20000 500 70 10.00001 1e6 400.0 20000 +1.0 +1.0];
...
sub=[sub xfix(iunfix)'+0.50];
slb=[slb xfix(iunfix)'-1.50];
...
if (dofit),
  [CCt_0,CCc_0,CCm_0,CMRO2t_0,t,CCat_0,Vmt_0]=myPO2est5c(sparms(parms2fit),tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:)]);
  [xx,xresn,xres,xexflag,xout,xlam,xjac]=lsqnonlin(@myPO2est5c,sparms(parms2fit),slb(parms2fit),sub(parms2fit),xopt,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:) PO2d2(:)*PO2d2_base]);
  [CCt,CCc,CCm,CMRO2t,t,CCa,Vmt]=myPO2est5c(xx,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:)]);
else,
  [CCt,CCc,CCm,CMRO2t,t,CCa,Vmt]=myPO2est5c(sparms(parms2fit),tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:)]);
end;
...
axis('tight'), grid('on'), fatlines;
subplot(614)
plot(t*60+CBFt(1),CCm/alpha_o2,'g')
ylabel('P_{mO2}'),
axis('tight'), grid('on'), fatlines;
subplot(615)
