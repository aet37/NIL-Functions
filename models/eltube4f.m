function [p,psi,eta,u,v,Vreal,Vinreal,Voutreal]=eltube4f(q,t,r,x,km,parms)
% Usage ... [p,psi,eta,u,v,vol,vin,vout]=eltube4f(q,t,r,x,km,parms)

% the parameter k is 2pi/wavelength and this should be O(1) over diameter
% of the vessel and approx close to the wave speed (~ 8m/s), so k~ 0.002 - 8.0
...
for m=1:length(q),
  ...
  [Xr,eta_n,psi_n,p_n,u_n,v_n]=eltube4fn(qf(m),f(m),km,parms,r,x,t);
  eta=eta+eta_n;
  psi=psi+psi_n;
  p=p+p_n;
  u=u+u_n;
