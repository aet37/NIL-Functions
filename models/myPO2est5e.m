function [CCt,CCc,CMRO2,t,PSt]=myPO2est5(x,tparms,sparms,parms2fit,iunfix,ifix,ttt,tdata,data)
% Usage ... [CCt,CCc,CMRO2,t,PSt]=myPO2est5(x,tparms,sparms,parms2fit,iunfix,ifix,ttt,tdata,data)
%
% x = parameters to fit
% tparms = [dt T]
...
PSt=PS*(1+ps_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t));

if (ps_type>1)
  RK_flag=0;
  VVc=Vc*((1+ps_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t)).^2); 
  if (ps_type==2),
    VVt=Vct-VVc;
  else,
    VVt=ones(size(VVc))*Vt;
  end;
end;

%disp(sprintf('  neglectplasma_flag= %f,  rk_flag= %d,  ps_type= %d',neglectplasma,RK_flag,ps_type));
%disp(sprintf('  ps_amp= %.3f   fp2cmro2f= %.3f',ps_amp,fp2cmro2_f)); 
...
for mm=2:length(t),
  ...
  B =  PSt(mm-1)*( CCp - CCt(mm-1) );
  CCc(mm) = CCc(mm-1) + (dt/Vc)*( A - B );
  CCt(mm) = CCt(mm-1) + (dt/Vt)*( B - CMRO2(mm-1) );

  if (ps_type>1),
  if (mm>2),
    dVVcdt = (VVc(mm-1)-VVc(mm-2))/dt;
    dVVtdt = (VVt(mm-1)-VVt(mm-2))/dt;
  else,
    dVVcdt=0; dVVtdt=0;
  end;
  CCc(mm) = CCc(mm-1) + (dt/VVc(mm-1))*( A - B - CCc(mm-1)*dVVcdt );
  CCt(mm) = CCt(mm-1) + (dt/VVt(mm-1))*( B - CMRO2(mm-1) - CCt(mm-1)*dVVtdt );
  end;
  ...
  if (RK_flag),
    ...
    PSa=PS*(1+ps_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)));
    PSb=PS*(1+ps_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)+dt/2));
    PSc=PS*(1+ps_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)+dt));
    Cck1=(dt/Vc)*( Fa*2*(Ca-CCc(mm-1)) - PSt(mm-1)*(CCp-CCt(mm-1)) );
    if (neglectplasma),
      Cck2=(dt/Vc)*( Fb*2*(Ca-(CCc(mm-1)+Cck1/2)) - PSb*(H_inv(CCc(mm-1)+Cck1/2,Hparms)-CCt(mm-1)) );
      Cck3=(dt/Vc)*( Fb*2*(Ca-(CCc(mm-1)+Cck2/2)) - PSb*(H_inv(CCc(mm-1)+Cck2/2,Hparms)-CCt(mm-1)) );
      Cck4=(dt/Vc)*( Fc*2*(Ca-(CCc(mm-1)+Cck3)) - PSc*(H_inv(CCc(mm-1)+Cck3,Hparms)-CCt(mm-1)) );
    else,
      Cck2=(dt/Vc)*( Fb*2*(Ca-(CCc(mm-1)+Cck1/2)) - PSb*(H_inv1(CCc(mm-1)+Cck1/2,Hparms)-CCt(mm-1)) );
      Cck3=(dt/Vc)*( Fb*2*(Ca-(CCc(mm-1)+Cck2/2)) - PSb*(H_inv1(CCc(mm-1)+Cck2/2,Hparms)-CCt(mm-1)) );
      Cck4=(dt/Vc)*( Fc*2*(Ca-(CCc(mm-1)+Cck3)) - PSc*(H_inv1(CCc(mm-1)+Cck3,Hparms)-CCt(mm-1)) );
    end;
    ...
    CMRO2c=CMRO2(mm);
    Ctk1=(dt/Vt)*( PSa*( CCp - CCt(mm-1)) - CMRO2a );
    Ctk2=(dt/Vt)*( PSb*( CCp - (CCt(mm-1)+Ctk1/2) ) - CMRO2b );
    Ctk3=(dt/Vt)*( PSb*( CCp - (CCt(mm-1)+Ctk2/2) ) - CMRO2b );
    Ctk4=(dt/Vt)*( PSc*( CCp - (CCt(mm-1)+Ctk3) ) - CMRO2c );
