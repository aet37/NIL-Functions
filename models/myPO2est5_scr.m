...
[CCt_0,CCc_0,CMRO2t_0,t,CCat_1]=myPO2est5(sparms(parms2fit),tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
%[CCt,CCc,CMRO2t,t,CCat]=myPO2est5(sparms(parms2fit),tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:) PO2d(:)*PO2d_base (PO2d2(:)-1)*PO2d2_base]);
[xx,xresn,xres,xexflag,xout,xlam,xjac]=lsqnonlin(@myPO2est5,sparms(parms2fit),slb(parms2fit),sub(parms2fit),xopt,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:) PO2d(:)*PO2d_base (PO2d2(:)-1)*PO2d2_base]);
[CCt,CCc,CMRO2t,t,CCa]=myPO2est5(xx,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
...
sparms5=sparms;
sparms6=sparms;
sparms1(10)=1.0*mean(PaO2np); sparms1(14)=0.0;
sparms2(10)=0.9*mean(PaO2np); sparms2(14)=0.0;
sparms3(10)=0.8*mean(PaO2np); sparms3(14)=0.0;
sparms4(10)=0.7*mean(PaO2np); sparms4(14)=0.0;
sparms5(10)=0.6*mean(PaO2np); sparms5(14)=0.0;
sparms6(10)=0.6*mean(PaO2np); sparms6(14)=0.1;

[CCt_1,CCc_1,CMRO2t_1,t,CCa_1]=myPO2est5(xx,tparms,sparms1,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
[CCt_2,CCc_2,CMRO2t_2,t,CCa_2]=myPO2est5(xx,tparms,sparms2,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
[CCt_3,CCc_3,CMRO2t_3,t,CCa_3]=myPO2est5(xx,tparms,sparms3,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
[CCt_4,CCc_4,CMRO2t_4,t,CCa_4]=myPO2est5(xx,tparms,sparms4,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
[CCt_5,CCc_5,CMRO2t_5,t,CCa_5]=myPO2est5(xx,tparms,sparms5,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
[CCt_6,CCc_6,CMRO2t_6,t,CCa_6]=myPO2est5a(xx,tparms,sparms6,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBFi(:) FPenv(:)]);
...
Pat_6=interp1(Ctot_curve,Cp_curve,CCa_6)/alpha_o2;
...
axis('tight'), grid('on'), fatlines;
subplot(312)
plot(t*60+CBFt(1),[CCt_1' CCt_2' CCt_3' CCt_4' CCt_5']/alpha_o2,PO2t,PO2d*PO2d_base,'k')
ylabel('P_{tO2} (Normal)'),
axis('tight'), grid('on'), fatlines;
legend('100% Part','90% Part','80% Part','70% Part','60% Part')
subplot(313)
...
ax=axis; axis([-5 60 ax(3:4)]);
subplot(335)
plot(kazu_res1(:,1),[kazu_res1(:,[2 4 6]) CCt_6'/alpha_o2 kazu_res1(:,10)])
ylabel('P_{tO2} Delivery (mmHg)'),
legend('100% Part','80% Part','60% Part','+10% Pinput','Est. Part')
fatlines; axis('tight'); grid('on'); dofontsize(15);
ax=axis; axis([-5 60 ax(3:4)]);
subplot(338)
plot(kazu_res1(:,1),[kazu_res1(:,[2 4 6]) CCt_6'/alpha_o2 kazu_res1(:,10)]+tmp2*ones(1,5),kazu_res1(:,1),kazu_res1(:,7),'k--')
ylabel('P_{tO2} (mmHg)'),
legend('100% Part','80% Part','60% Part','+10% Pinput','Est. Part','Meas.')
fatlines; axis('tight'); grid('on'); dofontsize(15);
ax=axis; axis([-5 60 ax(3:4)]);
subplot(332)
plot(kazu_res1(:,1),mean(PaO2np)*[1.0 0.8 0.6]'*ones(1,length(kazu_res1)),kazu_res1(:,1),Pat_6,kazu_res1(:,1),kazu_res1(:,9))
