function [S,S2,F2,F4,V3,CMRO2,EE,q,t,CCc,CCt]=myBOLD4b(x,tparms,sparms,parms2fit_i,iunfix,ifix,tdata,data)
% Usage ... [S,Fin,Fout,V,CMRO2,EE,q,t,Cc,Ct]=myBOLD4b(x,tparms,sparms,iparms,parms2fit_i,iunfix,ifix,tdata,data)
%
% x = parameters to fit
% tparms = [dt T]
...
for mm=2:length(t),
  ...
  SB(mm-1) = 1 + sk1*( 1 - sk2*((VV(mm-1)/VV(1))^bb2)*((qB(mm-1)/qB(1))^bb) );
  SBs(mm-1) = 1 + sk1*( 1 - sk2*((VV(mm-1)/VV(1))^bb2)*((qBs(mm-1)/qBs(1))^bb) );
  SBb(mm-1) = 1 + sk1*( 1 - sk2*((VV(mm-1)/VV(1))^bb2)*((qBb(mm-1)/qBb(1))^bb) );
