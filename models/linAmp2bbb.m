function [xx,y1,u1]=linAmp2bbb(x,t,foh_args,hrf,data,cbeta,tbias)
...
if isempty(x),
  x=[100 ones(1,size(hrf,2)) 2*rand(1,xlen1)];
end;

x0=ones(size(x));
xlb=[ 0   -1e2*ones(1,size(hrf,2)) -1*ones(1,xlen1)];
xub=[+1e4 +1e2*ones(1,size(hrf,2)) +1*ones(1,xlen1)];
...
function [y,uu]=linAmp2_wrap(x,t,foh_args,hrf,data,cbeta,ibias)
  ...
  uu=myfoh(ttfoh,ttfoh_iv,x(1+n_hrf+[1:xlen1]),ttfoh_if,0,t); 
  uu=uu(:)/sum(uu(:));
  for mm=1:n_hrf,
    yest=yest+x(mm)*myconv(uu(:),hrf(:,mm));
  end;
  yest=yest*x(1);
  y=yest;
  ...
  if nargin>4,
    ...
    if do_penalty,
      beta=cbeta;    % 2e-3
      cpen=0;
      cpen=abs(diff(x(1+n_hrf+[1:xlen1])));   
