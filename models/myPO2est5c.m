function [CCt,CCc,CCm,CMRO2,t,Cat,Vmt]=myPO2est5c(x,tparms,sparms,parms2fit,iunfix,ifix,ttt,tdata,data)
% Usage ... [CCt,CCc,CCm,CMRO2,t,CCa,Vm]=myPO2est5c(x,tparms,sparms,parms2fit,iunfix,ifix,ttt,tdata,data)
%
% x = parameters to fit
% tparms = [dt T]
...
Vm_amp=sparms(16);
ca_amp=sparms(17);
xunfix=sparms(18:end);

Cat=Ca*(1+ca_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t));

%Vmt=Vm*(1+Vm_amp*fp2cmro2_f*FP);
Vmt=Vm*(1+Vm_amp*myfoh(ttt,iunfix,xunfix,ifix(:,1),ifix(:,2),t));
Vtt=Vt+Vm-Vmt; %Vtt=Vt*ones(size(Vtt));
...
CCt(1)=Ct; CCc(1)=Cc; CCm(1)=Cm; EE(1)=E0;
for mm=2:length(t),
  ...
  if (mm>2), 
    dVmdt=(Vmt(mm)-Vmt(mm-1))/dt; 
    dVtdt=(Vtt(mm)-Vtt(mm-1))/dt;
  else,
    dVmdt=0;
    dVtdt=0;
  end;
  ...
  B =  PS*( CCp - CCt(mm-1) );
  D =  PSm*((Vmt(mm-1)/Vm)^0)*(CCt(mm-1) - CCm(mm-1) );
  CCc(mm) = CCc(mm-1) + (dt/Vc)*( A - B );
  CCt(mm) = CCt(mm-1) + (dt/Vtt(mm-1))*( B - D - dVtdt*CCt(mm-1) );
  CCm(mm) = CCm(mm-1) + (dt/Vmt(mm-1))*( D - CMRO2(mm-1) - dVmdt*CCm(mm-1) );
