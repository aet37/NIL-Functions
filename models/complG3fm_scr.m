...
%xg=[ 1    1    3   2.4   0     1    12   0.64  0.00  1.5/60 0.0257 1.54/60 11.5 0.52 1 1 1];
xg=[ 1    1    3.0   2.4   0     1    12   0.62  0.00  1.5/60 0.0253 1.35/60 11.7 0.64 1 1 1];
xl=[0.1  0.1  2.1  0.1     0    0.0  9.0  0.55   0  0.1/60 1e-5 0.1/60 1 0.2 0 0 0];
...
parms2fit=[3 8 9    12 13 14     15 16 17];
%parms2fit=[3 6 7];
...
xx=lsqnonlin(@complG3fm,xg(1,parms2fit),xl(parms2fit),xu(parms2fit),opt2,tparms,xg,parms2fit,[avgE1i;avgE2i;avgE3i],[W1;W2;W3]);
...
ee2=sum(((avgE2i-yy2).*W2).^2)/sum(W2);
ee3=sum(((avgE3i-yy3).*W3).^2)/sum(W3);

ee1n=ee1/max(avgE1i-1);
ee2n=ee2/max(avgE2i-1);
ee3n=ee3/max(avgE3i-1);
...
subplot(312)
plot(tt2*60,avgE2,'b--',t*60,yy2,'b-')
ylabel('BOLD'),
axis([tt2(1)*60 tt2(end)*60 min([avgE2 yy2]) max([avgE2 yy2])]), grid('on'),
fatlines, dofontsize(15);
subplot(313)
plot(tt3*60,avgE3,'b--',t*60,yy3,'b-')
