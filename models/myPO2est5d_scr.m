...
Vct=98.0;
PS=7000;
Pa=0.73*mean(PaO2lp);		% 0.73 or 0.95
Pt=mean(PO2lp_base);
bbeta=0.000;
fp2cmro2f=fp2f;
Caamp=0.00;
...
iunfix=find(~tss');
xramp=[4 4];
xfix=1.0*mytrapezoid3(ttt*60,10+xramp(1),12-xramp(1),xramp);


parms2fit=[length(sparms)+[1:length(iunfix)]];
sparms_orig=sparms;
sparms=[sparms_orig xfix(iunfix)'];
sub=[sub xfix(iunfix)'+2.0];
slb=[slb xfix(iunfix)'-2.0];




[CCt_0,CCc_0,CMRO2t_0,t,CCat_0]=myPO2est5d(sparms(parms2fit),tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:)]);
[xx,xresn,xres,xexflag,xout,xlam,xjac]=lsqnonlin(@myPO2est5d,sparms(parms2fit),slb(parms2fit),sub(parms2fit),xopt,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:) PO2d2(:)*PO2d2_base]);
[CCt,CCc,CMRO2t,t,CCa]=myPO2est5d(xx,tparms,sparms,parms2fit,iunfix,[ifix;xfix(ifix)']',ttt,(CBFt-CBFt(1))/60,[CBF2i(:) FP2env(:)]);
...
figure(2)
plot(PO2t,normch(2-PO2d2,1),t*60+PO2t(1),normch(CMRO2t/CMRO2t(1),1))
ylabel('Norm. Ch.')
legend('P_{tO2}','CMR_{O2} est.')
axis('tight'), grid('on'), fatlines;
