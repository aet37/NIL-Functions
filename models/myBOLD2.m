...
for mm=2:length(t),
  ...
  y(mm) = y(mm-1) + dt*( (1/ytau)*( myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)) - y(mm-1) ));
  ...
  elseif (c3type==4),
    y3(mm)=y3(mm-1)+dt*( (1/C3tau)*( myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1)) - y3(mm-1) ));
  ...
  if (nctype==1),
    y2uu=myfoh(tdata,iunfix,xunfix,ifix(:,1),ifix(:,2),t(mm-1));
  else,
    u2uu=mytrapezoid2(t(mm-1),y2st,y2dur,y2ramp,ytype);
    u2(mm) = u2(mm-1) + (dt/u2tau)*( u2uu - u2(mm-1) );
    y2uu=u2(mm-1);
  end;   
  y2(mm) = y2(mm-1) + dt*( (1/y2tau)*( y2uu - y2(mm-1) ));
  ...
  if (RK_flag),
    y2h = y2(mm-1) + (dt/2)*( (1/y2tau)*( y2uu - y2(mm-1) ));
    Finh = F0*(1+y2amp*y2h);
