...
elseif (oxlim==2),
  ...
  Deff_c=4.0;
  Deff_c=(0.5*(fin(1,2)+fout(1,2))/v(1,2))*log(1/(1-E0));
  Deff=Deff_c*v(:,2);
  E=1-exp(-Deff./(0.5*(fin(:,2)+fout(:,2))));
  %)E0=E(1);
else,
  E=ones(size(fin(:,3)));
  ke=ones(size(fin(:,3)));
...
q(1)=q0;
for m=1:length(fin(:,3))-1,
 q(m+1) = q(m) + dt*( (q0/v0)*(E(m)/E0)*fin(m,3) - (q(m)/v(m,3))*fout(m,3) );
end;
q=q(:);
rqin=(q0/v0)*(E/E0).*fin(:,3);
rqout=(q./v(:,3)).*fout(:,3);
