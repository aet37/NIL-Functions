...
xg2=[ 1    1    3   2.4   0     1    12   0.62  0.00  1.5/60 0.0204 1.54/60 12.9 0.52 1 1];
xl2=[0.1  0.1  0.5  0.1     0    0.0  9.0  0.55   0  0.1/60 1e-5 0.1/60 1 0.2 0 0];
xu2=[100  100  100  100    100    5   16.0  0.75   2    5/60 2  10/60 50 0.7 5 5];

tparms2=[dt tfin1 2];
%parms2fit2=[3 6];
%parms2fit2=[3 6 8 10];
%parms2fit2=[3 8 9   11 12 13 14   15 16];
parms2fit2=[3 8 9   11 13   15 16];
...
xx2=lsqnonlin(@complG3fm2,xg2(1,parms2fit2),xl2(parms2fit2),xu2(parms2fit2),opt2,tparms2,xg2,parms2fit2,[avgC2bi;avgC3bi],[avgE2bi;avgE3bi],[W2;W3]);

xg2(:,6)=xx2(end-1:end)';
%yy1=complG3f(xx(1,1:end-3),tparms2,xg2(1,:),parms2fit(1:end-3));
yy2=complG3f(xx2(1,1:end-2),tparms2,xg2(1,:),parms2fit2(1:end-2));
yy3=complG3f(xx2(1,1:end-2),tparms2,xg2(2,:),parms2fit2(1:end-2));
%yy1=complG3f(xg2(1,parms2fit),tparms2,xg2(1,:),parms2fit2);
%yy2=complG3f(xg2(2,parms2fit),tparms2,xg2(2,:),parms2fit2);
%yy3=complG3f(xg2(3,parms2fit),tparms2,xg2(3,:),parms2fit2);
...
subplot(221)
plot(tt2*60,avgC2b,'b--',t*60,yy2(1,:),'b-')
ylabel('FAIR'),
axis([tt2(1)*60 tt2(end)*60 min([avgC2b yy2(1,:)]) max([avgC2b yy2(1,:)])]), grid('on'),
fatlines, dofontsize(15);
subplot(222)
plot(tt3*60,avgC3b,'b--',t*60,yy3(1,:),'b-')
ylabel('FAIR'), xlabel('Time (s)'),
axis([tt3(1)*60 tt3(end)*60 min([avgC3b yy3(1,:)]) max([avgC3b yy3(1,:)])]), grid('on'),
fatlines, dofontsize(15);
...
%fatlines, dofontsize(15);
subplot(223)
plot(tt2*60,avgE2b,'b--',t*60,yy2(2,:),'b-')
ylabel('BOLD'),
axis([tt2(1)*60 tt2(end)*60 min([avgE2b yy2(2,:)]) max([avgE2b yy2(2,:)])]), grid('on'),
fatlines, dofontsize(15);
subplot(224)
plot(tt3*60,avgE3b,'b--',t*60,yy3(2,:),'b-')
ylabel('BOLD'), xlabel('Time (s)'),
axis([tt3(1)*60 tt3(end)*60 min([avgE3b yy3(2,:)]) max([avgE3b yy3(2,:)])]), grid('on'),
fatlines, dofontsize(15);
