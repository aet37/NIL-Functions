...
% This script will attempt to reproduce the simulations performed
% by Ogawa et al (Biophys J 64: 803 1993) where the signal due to deoxyHb
% in vessels and diffusing spins were simulated but here we are going
% to take one step further and not assume that the vessel itself has
% a susceptibility difference but allow the deoxyHb themselves to provide
% the susceptibility difference and in the regime where their concentration
% is high to approach Ogawa's results, the problems to encounter however
% are a few, for example, dealing with the amount of oxygenation
% is not going to be as general...

% In this simulation there are going to be spins and perturbers
% A two-compartment type model is going to be used: tissue and vessel
% Pertubers are going to be limited to exist only in the vessels
% while a population of the spins is going to be inside and outside
% The spins inside and outside are going to diffuse (random walk)
% while the perturbers are going to remain still
% This model initially is going to a static model, nothing inside the
% vessel is going to move yet...
...
n_jumps=100;
total_experiment_time=30e-3;		% ms
jump_t=total_experiment_time/n_jumps;
Dtau=jump_t;
mean_Diff_dist=sqrt(2*D*total_experiment_time);		% ???? not Dtau?
jump_x=mean_Diff_dist/n_jumps;
as=1.0*((2*D*Dtau)^(1/2));
...
for m=1:n_perturbers,
  ...
  p_space2(m,:)=rot(tmpv',[1 2 3],[thetax thetay thetaz])';
end;
  ...
  if (length(tmpbad)>1),
    for o=1:length(tmpbad),
      if (tmpbad(o)~=m),
        found=0;
        while(~found),
          tmpnew=rand([1 3]).*[xdim ydim zdim]-[xdim/2 ydim/2 zdim2];
...
for m=1:n_spins,
  for n=1:n_perturbers,
    ...
    B_contribution=b_sphere(B_r,B0,ap,1.0+p_deoxyratio(n)*deoxyHb_dX,1.0);
...
total_experiment_time=30e-3;				% ms
mean_Diff_dist=sqrt(2*D*total_experiment_time);
...
for m=1:n_jumps,
  ...
  for n=1:n_spins,
    ...
    for o=1:n_perturbers,
      B_contribution=0;
      B_contribution=b_sphere(s_finalspace(n,:)-p_space(o,:),B0,ap,1.0+p_deoxyratio(o)*deoxyHb_dX,1.0);
...
s_T2=ones([n_spins 1])*90e-3;			% ms
s_signal=ones([n_spins 1]);
s_signal=exp(-exp_TE./s_T2).*exp(-j*s_Diffphase);

total_signal=abs(sum(s_signal));
total_signalperspins=(1/n_spins)*total_signal;
