function [Xr,eta,psi,p,u,v,Vreal,Vinreal,Voutreal]=eltube4fn(qn,fn,c0,parms,r,x,t)
% Usage ... [Xr,eta,psi,p,u,v,Vreal,Vinreal,Voutreal]=eltube4fn(qn,fn,c0,parms,r,x,t)
% 
...
if (nargout>1),
  ...
  if (fn==0),
    ...
    pr= i*qn*8*mu/(pi*km*a0*a0*a0*a0);

    ur=(1/4)*(i*km*pr/mu)*( r.*r - a0*a0 );
    i0=find(r==0); if length(i0)~=0, rr=r; rr(i0)=ones(size(i0)); end;
    vr=((km*km*pr)/(4*mu))*( rr.*rr.*rr/4 - a0*a0*rr./2 - a0*a0*a0*a0./(4*rr) );
    i0=find(r==0); if length(i0)~=0, vr(i0)=zeros(size(i0)); end;

    Apsieta=[1/(a0*a0) i*km*nu/a0; -2*nu/(a0*a0) -i*2*km/a0];
    bpr=(pr*E0*h0/(1-nu*nu))*ones([2 1]);
  ...
  for n=1:length(t),
    for m=1:length(x),
      u(m,:,n)=ur.*exp(i*(km*x(m)-w*t(n)));
      v(m,:,n)=vr.*exp(i*(km*x(m)-w*t(n)));
      p(m,n)=pr.*exp(i*(km*x(m)-w*t(n)));
      psi(m,n)=psir.*exp(i*(km*x(m)-w*t(n)));
      eta(m,n)=etar.*exp(i*(km*x(m)-w*t(n)));
    end;
