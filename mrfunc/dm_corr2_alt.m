function f=dm_corr2(dmfile1,alt,slc,reference,section,dmfile2,dmfile3,dmfile4,dmfile5,dmfile6,dmfile7,dmfile8,dmfile9)
% Usage ... f=dm_corr2(dmfile,alt,slice,reference,section,dmfile2,dmfile3,...)
%
...
if dosect,
  ...
  for nn=1:nfiles,
    ...
    for k=1:nsections,
      for m=section(2*k-1)+alt:2:section(2*k)+alt,
  ...
  for nn=1:nfiles,
    %eval(['disp(dmfile',int2str(nn),');']);
    eval(['dmfid=fopen(dmfile',int2str(nn),',''r'');']);
    if dmfid<3, error(['Could not open file ',int2str(nn),'!']); end;
    for k=1+alt:2:dminfo(8),
      %disp(k),
