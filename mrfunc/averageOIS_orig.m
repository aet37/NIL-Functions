...
for mm=1:ntr,
  ...
  for nn=1:nper/nfa,
    tmpim1=mean(readOIS(fname2,[cnt+1:cnt+nfa]),3);
...
if (do_norm), 
  eval(sprintf('save %s.mat avgim baseim baseim2 noff ntr nper nfa do_norm do_zoomdn basediff dcim dcim_hdr',fname3));
else,
  eval(sprintf('save %s.mat avgim noff ntr nper nfa do_norm do_zoomdn dcim dcim_hdr',fname3));
end;
toc,
