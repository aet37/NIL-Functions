  ...
  for mm=1:length(tmp),
    ...
    if ~tmp_skip,
      ...
      if mm==1,
        ...
        myOISscr1_new(tmp(mm).name,tmpid,'do_load',[1:cam_nims],'do_crop','do_bin',cam_bin,'do_binfirst','do_motc','do_maskreg','do_intc');
        eval(sprintf('load %s_res crop_ii mask_reg',tmpid));
      else,
        disp(sprintf('  processing %02d: %s with id %s',mm,tmp(mm).name,tmpid));
        myOISscr1_new(tmp(mm).name,tmpid,'do_load',[1:cam_nims],'do_crop',crop_ii,'do_bin',cam_bin,'do_binfirst','do_motc','do_maskreg',mask_reg,'do_intc');
      end;
    else,
...
if ~isempty(tmp),
  for mm=1:length(tmp),
    ...
    if ~tmp_skip,
      ...
      if mm==1,
        disp(sprintf('  processing %02d: %s with id %s then loading',mm,tmp(mm).name,tmpid));
        myOISscr1_new(tmp(mm).name,tmpid,'do_load',[1:pm_nims],'do_crop','do_bin',pm_bin,'do_binfirst','do_motc','do_maskreg','do_intc');
        eval(sprintf('load %s_res crop_ii mask_reg',tmpid));
      else,
        disp(sprintf('  processing %02d: %s with id %s',mm,tmp(mm).name,tmpid));
        myOISscr1_new(tmp(mm).name,tmpid,'do_load',[1:pm_nims],'do_crop',crop_ii,'do_bin',pm_bin,'do_binfirst','do_motc','do_maskreg',mask_reg,'do_intc');
      end;
    else,
      disp(sprintf('  skipping %02d: %s, mat file found',mm,tmp(mm).name));
    end;
