function f=getanalyzetc(analyzeroot,nvols,slc,pix,ord,info)
% Usage ... f=getanalyzetc(analyzeroot,nvols,slice,pix,ord,info)
%
% info= [xdim ydim zdim]

imgfile=sprintf('%s_%04d.img',analyzeroot,1);
hdrfile=sprintf('%s_%04d.hdr',analyzeroot,1);
...
for m=1:nvols,
  ...
  imgfile=sprintf('%s_%04d.img',analyzeroot,m);
  hdrfile=sprintf('%s_%04d.hdr',analyzeroot,m);
