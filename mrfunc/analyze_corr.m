function f=analyze_corr(frootname,slc,reference,section,info)
% Usage ... f=analyze_corr(rootname,slice,reference,section,info)
%
...
if length(section)>=3,
  ...
  for k=1:nsections,
    for m=section(2*k-1):section(2*k),
      im=getanalyzeim(frootname,m,slc,info);
      sumim=sumim+im;
  ...
  for k=section(1):section(2),
    im=getanalyzeim(frootname,k,slc,info);
    sumim=sumim+im;
    count=count+1;
  end;
  meanim=(1/count)*sumim;  %image
