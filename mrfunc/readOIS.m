...
if (isstr(im)),
  if strcmp(im,'all'),
    im=[1:nf];
  elseif strcmp(im,'first'),
    im=1;
  elseif strcmp(im,'last'),
    im=nf;
  elseif strcmp(im,'info'),
    im=-1;
  elseif strcmp(im,'avg'),
    im=[1:nf];
    avg_flag=1;
  elseif strcmp(im(1:3),'raw'),
    im=str2num(im(4:end));
    raw_flag=1;
    disp(sprintf('Returning raw im#%d',im));
  else,
...
elseif (avg_flag),
  tmp=0; tmpim=0; tmptc=zeros([nf 1]);
  for mm=1:nf,
    tmpim=fread(fid,[nx ny],'int8');
    tmptc(mm)=mean(tmpim(:));
    tmp=tmp+tmpim;
  end;
  y=dimage+tmp/nf;
