function averageOIS3(fname,fps,ntr,noff,nper,nfa,zoomdn,basediff)
% Usage ... averageOIS3(fname,fps,ntrials,offset,trialdur,desiredres,zoomdown,basediff)
%           this one does global division
...
for mm=1:ntr,
  ...
  for nn=1:round(nper/nfa),
    ...
    for oo=1:length(tmpstk),
      ...
      tmpim1=tmpim1+double(tmpstk(oo).data)/avgimtc(cnt2);
    end;
    tmpim1=tmpim1/nfa;
