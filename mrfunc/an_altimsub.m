function [f,g]=an_altimsub(file,slc,tdim,avg_flag,sections,norm_flag)
% Usage ... f=an_altimsub(file,slice,tdim,avg_flag,sections,norm_flag)
%
...
for m=1:length(sections)/2,
  for n=1:(sections(2*m)-sections(2*m-1)+1)/2,
    im1=getanalyzeim(file,2*n-1+sections(2*m-1)-1,slc);
    im2=getanalyzeim(file,2*n+sections(2*m-1)-1,slc);
    if (norm_flag),
      dim(:,:,cnt)=(im1-im2)./im1;
