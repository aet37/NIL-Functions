function averageLSI(fname,fps,ntr,noff,nper,nfa,zoomdn,basediff,neig)
% Usage ... averageLSI(fname,fps,ntrials,offset,trialdur,desiredres,zoomdown,basediff,neig)
...
if (strcmp(fname(end-3:end),'.stk')|strcmp(fname(end-3:end),'.tif')|strcmp(fname(end-3:end),'.raw')),
...
for mm=1:ntr,
  ...
  for nn=1:nper/nfa,
    tmpim11=readOIS(fname2,[cnt+1:cnt+nfa]);
    for nnn=1:size(tmpim11,3),
      [tmpstd,tmpavg,tmpcon]=imstd_mex(tmpim11(:,:,mm),neig,1,1);
      tmpcon11(:,:,mm)=tmpcon;
    end;
    tmpim1=mean(tmpcon11,3);
...
if (do_norm), 
  eval(sprintf('save %s.mat avgim baseim baseim2 neig noff ntr nper nfa do_norm do_zoomdn basediff dcim dcim_hdr',fname3));
else,
  eval(sprintf('save %s.mat avgim neig noff ntr nper nfa do_norm do_zoomdn dcim dcim_hdr',fname3));
end;
toc,
