function dout=getSpectroEnv(data,t,wt,wr,wot,fs,wtype)
...
if ~exist('wtype','var'), wtype=1; end;
...
w_wid=round(fs*wt);
w_skip=round(fs*wot);
if wtype==1, ww=ones(w_wid,1);
elseif wtype==2, ww=hamming(w_wid); 
elseif wtype==3, ww=hanning(w_wid); 
end;
