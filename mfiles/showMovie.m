...
elseif length(subim)==2,
  if do_color, subim=mean(data(:,:,:,subim(1):subim(2)),4); else, subim=mean(data(:,:,subim(1):subim(2)),3); end;    
end;
if do_color,
...
if do_write,
  ...
  if exist('VideoWriter'),
    ...
    if ddim(end)>200, datavid.FrameRate=30; else, datavid.FrameRate=10; end;    %7,10,20,30
    vid.Quality=100;
    open(vid);
    do_newvideo=1;
...
if (do_write==0)|(do_write==2),
  ...
  for mm=1:nfr,
    if do_color,
      tmpim=zeros(size(data,1),size(data,2),3);
      tmpim=data(:,:,1:tmpz,mm);
      if isempty(wl),
        tmpim=imwlevel(tmpim-subim,[],1);
      else,
        tmpim=imwlevel(tmpim-subim,wl,1);
      end;
      figure(figh), image(tmpim),
    else,
      tmpim=data(:,:,mm)-subim;
      if isempty(wl),
        figure(figh), imagesc(tmpim),
        tmpim=imwlevel(tmpim,[],1);
      else,
        figure(figh), imagesc(tmpim,wl),
...
if do_write&do_newvideo,
  close(vid);
  disp(sprintf('  movie saved as %s at %d fps',movname,datavid.FrameRate));
end;
