...
% side note, by default the optimization range is limited to a factor of 2
%
% Ex. xx=myaffine2d_scr(im1,im2,[],'point-select');
...
do_manual=0;
do_ptselect=0;
if strncmp(mask,'point-select',12),
  do_ptselect=1;
  if length(mask)>12, ps_sz=abs(str2num(mask(13:end))); else, pt_sz=20; end;
  mask=[];
  im1dim=size(im1);
  im2dim=size(im2);
  clf,
  ...
  tmpim1sm=im_smooth(tmpim1,im1dim/pt_sz); tmpim1sm=tmpim1sm/max(tmpim1sm(:));
  tmpim2sm=im_smooth(tmpim2,im2dim/pt_sz); tmpim2sm=tmpim2sm/max(tmpim2sm(:));
...
if ~exist('x0','var'), 
    x0=[0 0 0 1 1];
    %x0=[0 0 0 im2dim(1)/im2dim(1) im1dim(2)/im2dim(2)];
end;
