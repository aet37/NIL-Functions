...
im1sz=size(im1);
if length(im1sz)>2,
  disp(sprintf('  segmenting based on correlation of %d images',im1sz(3)));
  do_corr=1;
  im1stk=squeeze(im1);
  im1=mean(im1stk,3);
  im1sz=size(im1);
  ...
  while (~tmpok),
    ...
    if tmpbut==102, %flash
      ...
      while(~tmpok3),
        for mm=1:4,
          figure(figh), clf
          show(im1_orig), drawnow, pause(0.1),
          im_overlay4(im1_orig,mask), colormap gray, axis image, drawnow, pause(0.1),
        end;
        [tmp1x,tmp1y,tmpbut]=ginput(1);
        tmprc0=[round(tmp1y) round(tmp1x)];
        if ~isempty(tmpbut), if tmpbut==1, tmpok3=1; end; end;
      end;       
    end;
    ...
    if do_corr,
      ...
      while(~tmpok2),
        ...
        figure(figh), im_overlay4(im1_orig,masktmp), drawnow,
      ...
      figure(figh), im_overlay4(im1_orig,tmpmask), drawnow,

      [im1,tmpix,tmpiy]=getImRegion(im1_orig,tmprc0,floor(subim1sz/2));
      disp(sprintf('  center: [%d, %d], span: [%d, %d] and [%d, %d]',tmprc0(1),tmprc0(2),tmpix(1),tmpix(end),tmpiy(1),tmpiy(end)));
...
maskl_cm=imCenterMass(maskl);
mask2l_cm=imCenterMass(mask2l);
maskl_new_cm=imCenterMass(maskl_new);
...
if do_struc,
  ...
  y.maskll=maskl_new;
  y.maskl_cm=maskl_cm;
  y.mask2l_cm=mask2l_cm;
  y.maskll_cm=maskl_new_cm;
  clear maskl
  maskl=y;
end;
