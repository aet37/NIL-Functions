...
% or
%   opt1=sprintf('''do_loadall'',''do_keepraw'',''do_movie'',''do_motc'',''do_intc'',''do_motcref'',[1 2]');
%   my2Pscr1_wrap('*TSeries*',[],opt1)
%
%   opt1=sprintf('''do_loadall'',''do_keepraw'',''do_movie'',''do_motc'',''do_realign'',2,''do_motcref'',[1 2]');
%   my2Pscr1_wrap('*TSeries*',[],opt1)
%
%   opt1=sprintf('''do_loadall'',''do_proj'',''do_imfilt'',''do_keepraw'',''do_movie''');
%   my2Pscr1_wrap('*ZSeries*',[],opt1)
%
%   opt1=sprintf('''do_loadall'',''do_proj''');
%   my2Pscr1_wrap('*Single*',[],opt1)
%
%   opt1=sprintf('''do_loadall''');
%   my2Pscr1_wrap('*Line*',[],opt1)
...
if isempty(flist),
  ...
  if isempty(tmpdir),
    ...
    for mm=1:length(tmpdir), tmpdir(mm).name(end+1)='/'; end;
  end;
...
if ischar(flist),
  ...
  for mm=1:length(tmpdir),
    tmpflist{mm}=[tmpdir(mm).folder,'/',tmpdir(mm).name,'/'];
  end;
  flist=tmpflist;
end;
...
if exist('tmpdir','var'),
  for mm=1:length(tmpdir),
    ...
    if strcmp(tmpids{mm}(5),'-'),
      tmpids{mm}=[tmpdir(mm).name(1:4),tmpdir(mm).name(9:10),tmpdir(mm).name(end-2:end)]; 
    else,
      tmpids{mm}=[tmpdir(mm).name(1:4),tmpdir(mm).name(end-3:end)]; 
    end
  end;
