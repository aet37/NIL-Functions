...
for mm=1:x1,
  cc=0;
  for nn=1:x2,
    cc=cc+sum(sum(a1(:,:,oo,(mm-1)*x1+nn),1),2);
  end;
  %cc,
  ...
  if (dothisrow),
    for nn=1:x2,
      if transp_flag==1,
        tmp=sprintf('%s a1(:,:,oo,%d)''',tmp,(mm-1)*x1+nn);
      elseif transp_flag==2,
        tmp=sprintf('%s flipud(a1(:,:,oo,%d))',tmp,(mm-1)*x1+nn);
      elseif transp_flag==4,
        tmp=sprintf('%s flipud(fliplr(a1(:,:,oo,%d)))',tmp,(mm-1)*x1+nn);
      elseif transp_flag==-1,
        tmp=sprintf('%s fliplr(a1(:,:,oo,%d))',tmp,(mm-1)*x1+nn);
      else,    
        tmp=sprintf('%s a1(:,:,oo,%d)',tmp,(mm-1)*x1+nn);
      end;
    end;
...
if nargout==0,
  if (nargin<2),
    if do_color, show(a), else, imagesc(a), end;
  else,
    %show(a,minmax)
    %a=minmax(1)*(a<minmax(1))+minmax(2)*(a>minmax(2))+a.*((a>minmax(1))&(a<minmax(2)));
    if do_color, show(a,minmax), else, imagesc(a,minmax), end;
  end;
  axis('image');
  axis('off'); 
  title(' ');
