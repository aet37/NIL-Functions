...
%     tc_pv4_pos.atc2(find(tc_pv4_pos.atc2>2))=nan;
%     plotmsd4([1:200],mean(tc_pv4_pos.atc2,2,'omitnan'),std(tc_pv4_pos.atc2,[],2,'omitnan'))
...
do_stderr=1;
if ischar(s),
  if strcmp(s,'stderr'), do_stderr=1; s=[]; end;
end;

if nargin==1,
  y=mean(x,2);
  s=std(x,[],2);
  if do_stderr, s=s/sqrt(size(x,2)); end;
  x=[1:length(y)]';
elseif ((nargin==3)&(isempty(s))),
  if size(x,1)==size(y,1),
    s=std(y,[],2); if do_stderr, s=s/sqrt(size(y,2)); end;
    y=mean(y,2);
  else,
    s=std(x,[],2); if do_stderr, s=s/sqrt(size(x,2)); end;
    cc1=y;
    y=mean(x,2);
    x=[1:length(y)];
  end;
