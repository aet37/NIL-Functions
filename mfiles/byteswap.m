...
if info(3),
  [header,cnt]=fread(fid,info(2),'char');
  if cnt~=info(2),
    fclose(fid);
...
if info(1)==0,
  [data,cnt]=fread(fid,inf,'char');
else,
  [data,cnt]=fread(fid,info(1)*size,'char');
  if cnt~=info(1)*size,
    fclose(fid);
...
cnt=fwrite(fidn,newdata,'char');
if cnt~=info(1)*size,
  fclose(fidn);
  error('Could not write all bytes to new file!');
end;
fclose(fidn);
disp(['New filename= ',newfile]);
