function plotmsd4e(x,y,s,cc1,leg)
% Usage ... plotmsd4e(x,y,s,c)
%
% Plots the vector y against vector x overlaying a shaded region
% by s divided by sqrt(n) using the color defined in c. The figure to plot should be
% cleared to avoid transparency problems.
%
% Ex. plotmsd4e([0:0.05:20],sin(2*pi*0.15*[0:0.05:20]+pi/3),ones(1,length([0:0.05:20]))*0.2)
...
elseif nargin==2,
  if length(x)==length(y),
    s=std(y,[],2); s=s/sqrt(size(y,2));
    y=mean(y,2);
  else,
    s=std(x,[],2); s=s/sqrt(size(x,2));
    cc1=y;
    y=mean(x,2);
    x=[1:length(x)];
  end;
