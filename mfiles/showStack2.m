function y=showStack2(stk,ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8,ip9,ip10,ip11,ip12,ip13,ip14)
% Usage ... showStack2(stk,options)
%
...
if ischar(stk),
  fname=stk;
  if (strcmp(fname(end-2:end),'raw')|strcmp(fname(end-2:end),'stk')|strcmp(fname(end-2:end),'tif')),
    stk=readOIS3(fname,'all');
  elseif strcmp(fname(end),'/'),
    stk=readPrairie2(fname);
  else,
    disp(sprintf('  error: could not find filename %s',fname));
  end;
end;
...
% uicontrol position [0.04 0.1 0.06 0.7]
% uicontrol position [0.04 0.8 0.06 0.9]
% set axis image
...
S.sl=uicontrol('style','slider',...
               'unit','pixels',...
               'position',[70 20 400 20],...
               'sliderstep',[1 10]./size(stk,length(size(stk))),...
               'min',1,'max',size(stk,length(size(stk))),'val',1);             
S.ed=uicontrol('style','edit',...
                 'unit','pixels',...
                 'position',[490 22 60 20],...
                 'fontsize',16,...
                 'string','1');
set([S.ed,S.sl],'call',{@ed_call,S});
%addlistener(S.sl,'call',{@ed_call,S.sl});
...
% Callback for the edit box and slider.
  ...
  switch H  % Who called?
    case S.ed
        ...
        L{3}=round(L{3});
        E=round(str2double(get(H,'string')));  % Numerical edit string.
        if E>=L{1} && E<=L{2}
        ...
        SS.mm=E;
    case S.sl
        set(S.ed,'string',round(get(H,'value'))) % Set edit to current slider.
        SS.mm=round(get(H,'value'));
    otherwise
        % Do nothing, or whatever.
  end
  if SS.do_color,
    myshow(SS.stk(:,:,:,SS.mm)), axis(SS.ax), xlabel(sprintf('%d/%d',SS.mm,SS.sdim(4))), drawnow;
  else,
    myshow(SS.stk(:,:,SS.mm)), axis(SS.ax), xlabel(sprintf('%d/%d',SS.mm,SS.sdim(3))), drawnow;
  end;
  guidata(H,SS)
  %uiresume(H);
