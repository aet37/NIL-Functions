...
% motcref = [im# ch#]
% maskreg_parms = maskl
% cycleconcat_parms = [#cycles] or 'select'
%
% ex. 
...
if flags.do_realign,
  ...
  if flags.realign_parms(1)==2,
    ...
    tmpin=input(sprintf('  continue? (0:skip, 1:yes, 3:interp-art-motcref-ch%d): ',motc_refch));
    ...
    if tmpin==3,
      if (length(datasz)>4)&flags.do_cycleswap,
        ...
        tmptcord=7; if length(tmptc)<100, tmptcord=1; end;
        tmptc=tcdetrend(tmptc,tmptcord,[1 length(tmptc)]); 
        tmptc=tmptc/mymode(tmptc)-1;
        clf,
        subplot(211), plot(xx_motc_all), 
        axis('tight'), grid('on'), ylabel('Displacement'),
  ...
  if flags.do_realign_artinterp,
    ...
    if isempty(tmpi2),
      ...
      if exist('xx_motc_all','var'),
        ...
        tmpii_motc_nan=find(isnan(xx_motc));
        if ~isempty(tmpii_motc_nan),
            disp(sprintf('  warning: found some nan in xx_motc (%d), replacing with original values...',length(tmpii_motc_nan)));
            xx_motc(tmpii_motc_nan)=xx_motc_orig(tmpii_motc_nan);
        end
        xx_motc_all=xx_motc;
        clear xx_motc
...
if flags.do_average,
  ...
  if isempty(flags.average_parms),
    ...
    ddim=size(data);
    ntr=flags.average_parms(1);
    nimtr=flags.average_parms(2);
    noff=flags.average_parms(3);
    nkeep=flags.average_parms(4);
    if flags.do_timing,
      adata=zeros(ddim(1),ddim(2),ddim(3),nimtr);
      ...
      avgtc_avg=squeeze(mean(mean(adata,1),2))';
      avgim_avg=mean(adata,3); 
      stdim_avg=std(adata,[],3);
