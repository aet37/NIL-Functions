...
if iscell(x)|iscell(y_vec),
    nmax=length(y_vec);
    nrows=ceil(sqrt(nmax));
    ncols=ceil(nmax/nrows);
    for mm=1:nmax,
        if iscell(x), thisx=x{mm}; elseif length(x)==1, thisx=[1:length(y_vec{mm})]; else, thisx=x; end;
        subplot(nrows,ncols,mm), plot(thisx,y_vec{mm}), axis tight, grid on,
        title(num2str(mm)),
    end
    return
end;
...
if nmax>pmax,
  disp(sprintf('  max# plots exceeded (%d), aborting',pmax));
  return;
end;
