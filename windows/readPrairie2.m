...
if strcmp(froot(end),filesep), do_addpath=1; else, do_addpath=0; end;

if iscell(froot),
  if length(froot)==1, froot{2}=' '; end;
  tmpstr=strsplit(froot{1},froot{2});
  tmproot=[tmpstr{1},filesep];
  disp(sprintf('  delimeter name modification\n  in-name= %s\n  out-name= %s',froot{1},tmproot));
  froot1=sprintf('''%s%s''',froot{1},filesep);
  froot=tmproot;
  do_addpath=1;
else
  if length(strfind(froot,filesep))>1 
      froot1=froot;
      tmpsplit=strsplit(froot, filesep);
      froot=[cell2mat(tmpsplit(end-1)) filesep];
