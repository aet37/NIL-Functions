function [vel,rnln,rsnr]=lsVel1(lineim,dxdt,lparms,sparms,rparms)
% Usage ... [vel,nln,snr]=lsVel1(lineim,dxdt,lparms,snrparms,rtype)
%
% lparms=[#rows #rowskip smw]
% snrparms=[Nsnr=12 SNRthr=3 fdiff=0.5 widN=1]
% dx_dt=[pixsize_in_mm linetime_in_sec];
% rtype=[default:4-Radon rmin rmax]
...
for nn=1:floor(ldim(1)/lnskp - (l1(2)-l1(1)+1)/lnskp),
  ...
  if do_smooth,
    tmpim1=im_smooth(lineim(tmpl1(1):tmpl1(2),l2(1):l2(2)),smw);
  else,
  ...
  if do_finetune,
    ...
    if abs(rang(nn)-rang2(nn))>0.1,
      disp('  replacing with fine-tune angle (%.4f to %.4f) ...',rang(nn),rang2(nn));
      rang(nn)=tmpang(tmpmaxi);
    end;
  end;
  ...
  % calculate velocity
  vel(nn)=dx_dt*cot(rang(nn)*pi/180);
