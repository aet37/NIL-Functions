...
if isfield(x,'FLUX'),
  ...
  if do_orig, y.FLUX_orig=x.FLUX; end;
end;
if isfield(x,'TB'),
  for mm=1:ddim(2),
    tmpTB=x.TB(:,mm);
    ...
    if interp_flag,
      ...
      tmpTBi=interp1(tt,mysmooth(tmpTB,fs(2)),tti);
      clear tmpTB
      tmpTB=tmpTBi;
    end;
...
if isfield(x,'FLUX')&exist('skp'),
  ...
  tmpflux=tmpflux./(ones(size(tmpflux,1),1)*mean(tmpflux(find(tt<-0.1),:),1));
  tmptb=tmptb./(ones(size(tmptb,1),1)*mean(tmptb(find(tt<-0.1),:),1));
  tmpflux=mean(tmpflux,2);
  tmptb=mean(tmptb,2);
  
  ps_tb_thr=1.02; ps_skp=150; ps_n=10; ps_end=5000; ps_start=1000;
  tmpii=find(tmptb(ps_start+1:end-ps_end)>ps_tb_thr);
  if ~isempty(tmpii),
    tmpii=tmpii+ps_start;
    tmpp0=polyfit([mean(tt(tmpii(1)-ps_skp+[-ps_n:ps_n])) mean(tt(tmpii(end)+ps_skp+[-ps_n:ps_n]))],...
                  [mean(tmpflux(tmpii(1)-ps_skp+[-ps_n:ps_n])) mean(tmpflux(tmpii(end)+ps_skp+[-ps_n:ps_n]))],1);
    tmpp1=polyfit([mean(tt(tmpii(1)+ps_skp+[0:ps_n])) mean(tt(tmpii(end)-ps_skp+[0:ps_n]))],...
                  [mean(tmpflux(tmpii(1)+ps_skp+[0:ps_n])) mean(tmpflux(tmpii(end)-ps_skp+[0:ps_n]))],1);
    tmppv0=polyval(tmpp0,tt(:));
    tmppv1=polyval(tmpp1,tt(:));

    tmpfluxnew=tmpflux;
    %tmpfluxnew(tmpii)=tmpflux(tmpii)-tmppv1(tmpii)+tmppv0(tmpii(1))+(tmppv0(tmpii)-tmppv0(tmpii(1)));
    tmpfluxnew(tmpii)=tmpflux(tmpii)-tmppv1(tmpii)+tmppv0(tmpii);
    y.FLUXnew=tmpfluxnew;
    
    if detrend_flag,
      tmpFLUX=y.FLUXnew*mean(y.FLUXbase);
      tmpp=polyfit(tt(dii),tmpFLUX(dii),dord);
      tmpv=polyval(tmpp,tt);
      tmpFLUX=tmpFLUX-tmpv+mean(tmpv);
      y.FLUXnew=tmpFLUX/mean(tmpFLUX(find(tt<-0.1)));
    end;
    if interp_flag,
     y.FLUXnew_i=interp1(tt,y.FLUXnew,tti);
    end;
  end;
  ...
  %tmpflux=tmpflux./(ones(size(tmpflux,1),1)*mean(tmpflux(find(tt<-0.1),:),1));
  %tmptb=tmptb./(ones(size(tmptb,1),1)*mean(tmptb(find(tt<-0.1),:),1));
  %for nn=1:size(tmpflux,2),
  %  tmpii=find(tmptb(1:end-ps_end,nn)>ps_tb_thr);
  %  tmpp0=polyfit([mean(tt(tmpii(1)-ps_skp+[-ps_n:ps_n])) mean(tt(tmpii(end)+ps_skp+[-ps_n:ps_n]))],...
  %                [mean(tmpflux(tmpii(1)-ps_skp+[-ps_n:ps_n],nn)) mean(tmpflux(tmpii(end)+ps_skp+[-ps_n:ps_n],nn))],1);
  %  tmpp1=polyfit([mean(tt(tmpii(1)+ps_skp+[0:ps_n])) mean(tt(tmpii(end)-ps_skp+[0:ps_n]))],...
  %                [mean(tmpflux(tmpii(1)+ps_skp+[0:ps_n],nn)) mean(tmpflux(tmpii(end)-ps_skp+[0:ps_n],nn))],1);
  %  tmppv0=polyval(tmpp0,tt(:));
  %  tmppv1=polyval(tmpp1,tt(:));
  %  tmpfluxnew=tmpflux(:,nn);
  %  %tmpfluxnew(tmpii)=tmpflux(tmpii)-tmppv1(tmpii)+tmppv0(tmpii(1))+(tmppv0(tmpii)-tmppv0(tmpii(1)));
  %  tmpfluxnew(tmpii)=tmpflux(tmpii,nn)-tmppv1(tmpii)+tmppv0(tmpii);
  %  y.FLUXnew_all(:,nn)=tmpfluxnew;
  %end;  
end;
